...
function Populate-AzureSubscriptions
{
        ...
        $subscriptionARM = @((Get-AzSubscription -ErrorAction SilentlyContinue).Name )
    ...
    $UI_combo_Subscriptions.SelectedIndex = $UI_combo_Subscriptions.items.IndexOf((Get-AzContext).Subscription.Name)
}

function Populate-AzureEnvironments
{
    $UI_combo_Environment.ItemsSource= @((Get-AzEnvironment).Name )
...
function filter-Results 
{
 ...
 $UI_button_LoadSubscription.add_Click({
     ...
     Get-AzSubscription -SubscriptionName $UI_combo_Subscriptions.SelectedItem | Select-AzureRmSubscription
     load-azurevmlist
