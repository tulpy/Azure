...
.LINK
    https://github.com/JamesDLD/Azure-PowerShell
.EXAMPLE
...
{
    $Action = "Getting the Azure context for the SubscriptionName : $($AzSubscription.Name)"
    $Command = {Get-AzSubscription -SubscriptionName $AzSubscription.Name | Set-AzContext -ErrorAction Stop}
    $AzContext = Generate_Log_Action -Action $Action -Command $Command -LogFile $logFile

    $Action = "Selecting the Azure SubscriptionName : $($AzSubscription.Name)"
    $Command = {Select-AzSubscription -Name $AzSubscription.Name -Context $AzContext -Force -ErrorAction Stop}
    $Result = Generate_Log_Action -Action $Action -Command $Command -LogFile $logFile

    $Action = "Getting the vnet from the SubscriptionName : $($AzSubscription.Name)"
    $Command = {Get-AzVirtualNetwork -ErrorAction Stop}
    $Vnets = Generate_Log_Action -Action $Action -Command $Command -LogFile $logFile

    foreach ($Vnet in $Vnets)
    {
        $Action = "Getting the vnet peering limit on the region : $($Vnet.Location) on the SubscriptionName : $($AzSubscription.Name)"
        ...
        $VnetSummary_array += VnetSummary_array -AzureSubscriptionName $AzSubscription.Name `
            -VnetName $Vnet.Name `
